...
class TestPosException(TestPosOrder):
    ...
    def test_pos.order_exception(self):
        self.pos.exception_confirm = self.env['pos.exception.confirm']
        exception = self.env.ref('pos.exception.excep_no_zip')
        ...
        # Simulation the opening of the wizard pos.exception_confirm and
        # set ignore_exception to True
        so_except_confirm = self.pos.exception_confirm.with_context(
            {
                'active_id': so1.id,
                'active_ids': [so1.id],
